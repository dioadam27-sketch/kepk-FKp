...
const decodedStr = "ZXWV+?KF4xrddMT_?TLBCFBB+S:D^1;[[HO\\*J]DH?TLBC=RG@=6WT]%L"; // Sample from screenshot

let keyChars = [];
for (let i = 0; i < decodedStr.length; i++) {
  let hexByte = parseInt(hexData.substring(i*2, i*2+2), 16);
  let decByte = decodedStr.charCodeAt(i);
  keyChars.push(String.fromCharCode(hexByte ^ decByte));
}
console.log("Frontend is currently using this key:", keyChars.join(''));
